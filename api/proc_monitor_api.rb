  ...
  class ProcMonitor < Grape::API
    ...
    namespace :proc_monitor do
        ...
        params do
            ...
            requires :cycle, type: String, desc: "monitor cycle", regexp: /^['"]?10|60|600|3600|21600|86400['"]?$/
        ...
        params do
            ...
            requires :monitor_item, type: String, desc: "monitor_item", regexp: /^['"]?[A-Za-z_][A-Za-z0-9_\-.]*['"]?$/
            requires :compare, type: String, desc: "compare conditions: eg < <= !=....", regexp: /^['"]?>|>=|==|!=|<|<=['"]?$/
            requires :threshold, type: String, desc: "threshold", regexp: /^['"]?\d+['"]?$/
            requires :filter, type: String, desc: "setting for alarm strategy", regexp: /^['"]?\d\/\d['"]?$/
            requires :disable_time, type: String, desc: "disable time in one day", regexp: /^['"]?\d\d:\d\d:\d\d\-\d\d:\d\d:\d\d['"]?$/
        end
        get '/add_rule' do
            raw_key=format(params['raw_key'])
            if ProcMonitorRaw.where(:raw_key=>raw_key).empty?
        ...
        params do
            requires :raw_key, type: String, desc: "raw key"
            requires :max_alert_times, type: String, desc: "max alert times", regexp: /^['"]?\d?\d['"]?$/
